...
public class ExpressionTree
{
  ...
  internal IEnumerable<(ITestGroup group, List<ITestFunction> functions)> GetFilteredTests(
    UnitTestManager unitTestManager)
  {
    if (root == null)
    {
      foreach ((ITestGroup, List<ITestFunction>) filteredGroup in
        unitTestManager.UnitTests.OrderBy(group => group)
         .Select(group => (group, group.TestFunctions.ToList())))
        yield return filteredGroup;
      yield break;
    }

    foreach (UnitTestGroup group in unitTestManager.UnitTests.OrderBy(group => group))
    {
  ...
  {
    public abstract bool Evaluate(ITestGroup group, ITestFunction function);
  }
  ...
  public class ExpressionNode : Node
  {
    ...
    public override bool Evaluate(ITestGroup group, ITestFunction function)
    {
